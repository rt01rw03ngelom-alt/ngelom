...
const APP_VERSION = '1.2.6'; // Tingkatkan versi ini setiap kali update
...
self.addEventListener('push', (event) => {
  event.waitUntil((async () => {
    try {
      // Default payload jika push tidak membawa data
      let payload = {
        title: 'Portal RT Ngelom',
        body: 'Ada pengumuman terbaru untuk Anda.',
        targetPage: 'home'
      };
      ...
      const options = {
        ...
        vibrate: [200, 100, 200],
        data: {
          url: `${self.location.origin}${self.location.pathname}?redirect=${payload.targetPage}`
        },
        tag: 'portal-rt-notif', 
        renotify: true
      };
...
event.respondWith((async () => {
    ...
    // 10XTHINK FIX: Jangan interupsi request ke Google Script API
